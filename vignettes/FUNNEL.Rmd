...
Users may customize their own studies by varing the default setting in `FUNNEL.GSEA()`. Details about the default parameter setting can be found in `help("FUNNEL.GSEA")`. The following code is a simple example, which uses 2 eigenfunctions and different panelty values for the elastic-net regression. Many of the immune signaling pathways remain significant.
...
```{r plotWeight2,  fig.width=5, fig.height=8}
...
1. We recommend to standardize time (into [0,1] interval), so that the smoothing penalty is comparable across different data sets and the default value (`lambda=10^-3.5`) make sense in these situations. 
